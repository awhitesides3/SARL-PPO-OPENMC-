...
if __name__ == "__main__":
    ...
    onlyL1 = np.array([10.0,0.01])
    onlyL2 = np.array([0.01,10.0])
    tot_num = int(mcrs)
    rand_num = tot_num - 4
    rng = np.random.default_rng(42)
    x_rand = lb + (ub-lb) * rng.random((rand_num, 2))
    x_train = np.vstack([lb, onlyL1, onlyL2, x_rand, ub])
    reward_train = []
    dose_train = []
    cost_train = []
    for x in x_train:
    ...
    np.savez('surrogate_results/2_layer_sensitivity_runs/data/TEST'+npzFile+'.npz', x_train=x_train, reward_train=reward_train, dose_train=dose_train, cost_train=cost_train)
