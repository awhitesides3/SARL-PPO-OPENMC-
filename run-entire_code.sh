...
echo "Particles (nps): $nps" >> "$out_file"
echo "Dose constraint: $dose_constraint" >> "$out_file"
echo "-------------------------------------" >> "$out_file"
# Run Surrogate
python -u create_surrogate.py $ARGS_surrogate >> "$out_file" 2>&1 
# Log ppo
echo "-------------------------------------" >> "$out_file"
echo "Starting ppo run: $(date)" >> "$out_file"
echo "Finding optimal design from this data set: $save_path-surrogate_data.npz" >> "$out_file"
echo "-------------------------------------" >> "$out_file"
# Run PPO
