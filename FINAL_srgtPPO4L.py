...
cost_list = []
reward_list = []
shield_list = np.array([0.0,0.0,0.0,0.0])
layers = []
batches = None
tally_name = None
iteration = 1
def openmc_fitness(individual):
    ...
    thicknesses = np.clip(individual, [0.01, 0.01, 0.01, 0.01], [10.0, 10.0, 10.0, 10.0])
...
if __name__ == "__main__":
    ...
    onlyL1 = np.array([10.0,0.01,0.01,0.01])
    onlyL2 = np.array([0.01,10.0,0.01,0.01])
    onlyL3 = np.array([0.01,0.01,10.0,0.01])
    onlyL4 = np.array([0.01,0.01,0.01,10.0])
    tot_num = int(mcrs)
    rand_num = tot_num - 6
    rng = np.random.default_rng(42) #random seed number 42, don't change this for sensitivity studies.
    x_rand = lb + (ub-lb) * rng.random((rand_num, 4))
    x_train = np.vstack([lb, onlyL1, onlyL2, onlyL3, onlyL4, x_rand, ub])
    reward_train = []
    dose_train = []
    cost_train = []
    for x in x_train:
        reward, dose, cost = openmc_fitness(x)
    ...
    np.savez('surrogate_results/4_layer_sensitivity_runs/data/'+npzFile+'.npz', x_train=x_train, reward_train=reward_train, dose_train=dose_train, cost_train=cost_train)