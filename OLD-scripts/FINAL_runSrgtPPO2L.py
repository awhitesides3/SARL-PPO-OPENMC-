...
from FINAL_srgtPPO2L import npzFile as npz
...
###############################'''BEGIN CODE'''########################################
# load data
data = np.load('surrogate_results/2_layer_sensitivity_runs/data/'+npzFile+'.npz')
...
while not accuracy_check:
    ...
    print("Predicted Optimal Thicknesses [t1, t2]:", best_thicknesses)
...
new_directory = '/home/awhitesides3/openneomc/pporuns/surrogate_results/2_layer_sensitivity_runs/'+resultsFile
...
x_arr = np.column_stack([T1.ravel(), T2.ravel()])
mean, std = GP_dose.predict(x_arr, return_std=True)
mean = mean.reshape(T1.shape)
std = std.reshape(T1.shape)
mean_c, std_c = GP_cost.predict(x_arr, return_std=True)
mean_c = mean_c.reshape(T1.shape)
std_c = std_c.reshape(T1.shape)
# plot GP mean dose
plt.figure(10)
plt.contour(T1, T2, mean, levels=50, cmap='viridis')
plt.colorbar(label="Predicted Dose")
plt.contour(T1, T2, mean, levels=[dose_c], colors='red', linewidths=2, linestyles='--', label='Dose Limit')
plt.xlabel("Layer 1 thickness")
plt.ylabel("Layer 2 thickness")
plt.title("GP Mean Dose Prediction")
# overlay OpenMC runs
plt.scatter(x_train[:,0], x_train[:,1], c="red", s=30, label="OpenMC Samples")
plt.scatter(best_thicknesses[0], best_thicknesses[1], c="green", s=30, label="Gp Optimal Solution")
plt.legend()
plt.savefig(new_directory+'/'+"GP_dose_mean.png")
plt.close()
# plot GP dose uncertainty
plt.figure(11)
plt.contour(T1, T2, std, levels=50)
plt.colorbar(label="GP Std. Dev.")
plt.xlabel("Layer 1 thickness")
plt.ylabel("Layer 2 thickness")
plt.title("GP Dose Prediction Uncertainty")
plt.scatter(x_train[:,0], x_train[:,1], c="red", s=30, label="OpenMC Samples")
plt.scatter(best_thicknesses[0], best_thicknesses[1], c="green", s=30, label="Gp Optimal Solution")
plt.legend()
plt.savefig(new_directory+'/'+"GP_dose_uncertainty.png")
plt.close()
# plot GP mean cost
plt.figure(12)
plt.contour(T1, T2, mean_c, levels=50)
plt.colorbar(label="Predicted Cost")
plt.xlabel("Layer 1 thickness")
plt.ylabel("Layer 2 thickness")
plt.title("GP Mean Cost Prediction")
# overlay OpenMC runs
plt.scatter(x_train[:,0], x_train[:,1], c="red", s=30, label="OpenMC Samples")
plt.scatter(best_thicknesses[0], best_thicknesses[1], c="green", s=30, label="Gp Optimal Solution")
plt.legend()
plt.savefig(new_directory+'/'+"GP_cost_mean.png")
plt.close()
# plot GP cost uncertainty
plt.figure(13)
plt.contour(T1, T2, std_c, levels=50)
plt.colorbar(label="GP Std. Dev.")
plt.xlabel("Layer 1 thickness")
plt.ylabel("Layer 2 thickness")
plt.title("GP Cost Prediction Uncertainty")
plt.scatter(x_train[:,0], x_train[:,1], c="red", s=30, label="OpenMC Samples")
plt.scatter(best_thicknesses[0], best_thicknesses[1], c="green", s=30, label="Gp Optimal Solution")
plt.legend()
plt.savefig(new_directory+'/'+"GP_cost_uncertainty.png")
plt.close()
