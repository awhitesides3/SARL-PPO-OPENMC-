...
cost_list = []
reward_list = []
shield_list = np.array([0.0,0.0,0.0])
layers = []
batches = None
tally_name = None
iteration = 1
def openmc_fitness(individual):
    ...
    thicknesses = np.clip(individual, [0.01, 0.01, 0.01], [10.0, 10.0, 10.0])
...
def cost_calc(thicknesses):
    prices = np.array([0.0093, 3.70, 0.0093]) #prices of water and ss-316L taken from the dataset.jsons 
...
if __name__ == "__main__":
    ...
    onlyL1 = np.array([10.0,0.01,0.01])
    onlyL2 = np.array([0.01,10.0,0.01])
    onlyL3 = np.array([0.01,0.01,10.0])
    tot_num = int(mcrs)
    rand_num = tot_num - 5
    rng = np.random.default_rng(42)
    x_rand = lb + (ub-lb) * rng.random((rand_num, 3))
    x_train = np.vstack([lb, onlyL1, onlyL2, onlyL3, x_rand, ub])
    reward_train = []
    dose_train = []
    cost_train = []
    for x in x_train:
    ...
    np.savez('surrogate_results/3_layer_sensitivity_runs/data/'+npzFile+'.npz', x_train=x_train, reward_train=reward_train, dose_train=dose_train, cost_train=cost_train)
