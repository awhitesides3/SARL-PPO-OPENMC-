...
def openmc_fitness(individual):
    ...
    # else:
    #     penalty = 0
    penalty = max(0, (dose - dose_c)*1000/dose_c) #negative = 0
    reward = -cost - penalty
...
if __name__ == "__main__":
    ...
    tot_num = 10
    rand_num = tot_num - 2
    # x_train = lb + (ub-lb) * np.random.rand(num, 4)
    rng = np.random.default_rng(42)
    x_rand = lb + (ub-lb) * rng.random((rand_num, 2))
    x_train = np.vstack([lb, x_rand, ub])
    reward_train = []
    dose_train = []
    cost_train = []
    for x in x_train:
    ...
    np.savez("surrogate_results/2_layer_sensitivity_runs/bounded0.1-10-0-1-1e3-1e3.npz", x_train=x_train, reward_train=reward_train, dose_train=dose_train, cost_train=cost_train)
